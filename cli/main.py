...
@cli.command(
    "train",
    short_help="Run TRAIN for a MODEL for an input DATASET using specified PARAMETERS",
)
@click.option(
    "-m",
    "--metrics",
    type=click.Choice(choices=MODELS, case_sensitive=False),
    required=True,
)
@click.option(
    "-dp",
...
@click.option("-g", "--gpu", type=bool, default=True, help="Flag to train using GPU")
@click.option(
    "-ng",
    "--n-gpus",
    type=int,
