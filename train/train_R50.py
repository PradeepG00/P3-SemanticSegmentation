...
from mscg.loss.acwloss import ACWLoss
...
train_args = MSCGTrainingConfiguration(net_name='MSCG-Rx50',
                                       data='Agriculture',
                                       bands_list=['NIR', 'RGB'],
                                       kf=0, k_folder=0,
...
train_args.print_freq = 5
train_args.save_pred = False
# output training configuration to a text file
train_args.checkpoint_dir = os.path.abspath(os.curdir)
...
def random_seed(seed_value, use_cuda=True):
    print(
        inspect.currentframe().f_code.co_name)  # DEBUG # if CONFIG["TRACING_ENABLED"]: logging.debug(inspect.currentframe().f_code.co_name) # DEBUG
...
def train_rx50(device: str = "gpu"):
    # with open( Path(train_args.checkpoint_dir) / "train-rx.json", )
    ...
    # DEBUG using CPU
    if device.lower() == "gpu":
        logging.debug("model configured for GPU training")
        net.cuda()
    net.train()
    ...
    # DEBUG using CPU
    if device.lower() == "gpu":
        logging.debug("loss function configured for GPU training")
    ...
    while True:
        try:
            ...
            for i, (inputs, labels) in enumerate(train_loader):
                ...
                # DEBUG using CPU
                if device.lower() == "gpu":
                    logging.debug("input and labels configured for GPU training")
                ...
                print(
                    f"epoch {start_epoch + new_ep}, iter {curr_iter} / {num_iter}, loss {train_main_loss.avg}, aux {aux_train_loss.avg}, cls {cls_train_loss.avg},lr {optimizer.param_groups[0]['lr']}")
...
def update_checkpoint(net, optimizer, epoch, new_ep, val_loss,
    ...
    snapshot_name = 'rx50-epoch_%d_loss_%.5f_acc_%.5f_acc-cls_%.5f_mean-iu_%.5f_fwavacc_%.5f_f1_%.5f_lr_%.10f' % (
    ...
    if updated or (train_args.best_record['val_loss'] > avg_loss):
        print("Saving model checkpoint at:", os.path.join(train_args.save_path, snapshot_name + '.pth'))
    ...
    if train_args.save_pred:
        if updated or (new_ep % 5 == 0):
            val_visual = visualize_checkpoint(epoch, new_ep, inputs_all, gts_all, predictions_all)

    if len(val_visual) > 0:
        val_visual = torch.stack(val_visual, 0)
        val_visual = tv_utils.make_grid(val_visual, nrow=3, padding=5)
        writer.add_image(snapshot_name, val_visual)


def visualize_checkpoint(epoch, new_ep, inputs_all, ground_truth_all, predictions_all):
    print(
        inspect.currentframe().f_code.co_name)  # DEBUG # if CONFIG["TRACING_ENABLED"]: logging.debug(inspect.currentframe().f_code.co_name) # DEBUG
    val_visual = []
    if train_args.save_pred:
